...
?>
                                <tr>
                                    ...
                                    <td id="td1_<?php echo $row['mf_id']; ?>">
                                        <a href="#"
                                            onClick="friendProfile('<?php echo $friend_type; ?>','<?php echo $friend_index; ?>')"><?php echo $friend_name; ?>
                                        </a><br>
                                    ...
                                    <td>
                                        <!--MSK-00146 -->
                                        <a href="#"
                                            onClick="showModal('<?php echo $my_index; ?>','<?php echo $conversation_id; ?>','<?php echo $friend_index; ?>','<?php echo $my_type; ?>')"
                                            class="btn btn-primary btn-xs">Send Messege</a>
                                    </td>
    ...
    function showModal(my_index, conversation_id, friend_index, my_type) {
        ...
        xhttp.open("GET", "conversation_history_admin.php?my_index=" + my_index + "&my_type=" + my_type +
            "&conversation_id=" + conversation_id + "&friend_index=" + friend_index, true);
    ...
    document.onkeydown = function(evt) { //start press Enter Key
        ...
        if (keyCode == 13) {
            ...
            if (msg !== ' ') {
                ...
                xhttp1.open("GET", "../model/add_message.php?conversation_id=" + conversation_id + "&user_index=" +
                    my_index + "&msg=" + msg + "&user_type=" + user_type + "&do=" + do1, true);
                xhttp1.send();

                if (timer1 == 0) {
                    intervalID = setInterval(function() {

                        $('#chatRoom').load("conversation_history_admin1.php?my_index=" + my_index +
                            "&conversation_id=" + conversation_id + "&friend_index=" + friend_index + ""
                            );
    ...
    function showCurrentmsg(my_index, conversation_id, friend_index) {
        ...
        xhttp2.open("GET", "conversation_history_admin1.php?my_index=" + my_index + "&conversation_id=" +
            conversation_id + "&friend_index=" + friend_index, true);
        xhttp2.send();
    ...
    function msgRead(conversation_id, friend_index, my_index, my_type) {
        ...
        xhttp.open("GET", "../model/confirm_msg_read.php?conversation_id=" + conversation_id + "&friend_index=" +
